...
class Custom_Dataset(Dataset):
    def __init__(
            ...
            if '.csv' in self.dataset_name:
                ...
                if valid_subset_path:
                    ...
                    try:
                        ...
                        splits = self.dataset_name.split('-')
                        prefix = '-'.join(splits[:-1])
                        l1, l2 = splits[-1].split('_')
                        new_name = f'{prefix}-{l2}_{l1}'
                        dataset = load_dataset(
                            new_name,
                            split=type_path,
                            ignore_verifications=True,
        ...
        if any(d in self.dataset_name for d in DIALOG_DATASETS):
            self.dataset = self.dataset[self.dataset['text'].map(
                lambda d: len(d)) > 1]
    ...
    def convert_to_features(self, example_batch):
        ...
        if not task:
            if self.valid_subset_path:
                task = f'{self.dataset_name}/{self.valid_subset_path}'
            else:
